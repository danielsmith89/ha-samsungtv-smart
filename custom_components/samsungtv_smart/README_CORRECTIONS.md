...
Cette version corrigée du custom component `samsungtv_smart` inclut plusieurs améliorations importantes pour le téléchargement des thumbnails Frame Art Gallery.
...
cp -r samsungtv_smart samsungtv_smart.backup
```
...
cd /config/custom_components
# Supprimer ancien
rm -rf samsungtv_smart
# Copier nouveau
unzip samsungtv_smart_fixed.zip
```
...
```bash
grep -n "DRM-protected" /config/custom_components/samsungtv_smart/media_player.py
```
